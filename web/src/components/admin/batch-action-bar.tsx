...
export function BatchActionBar({ selectedCount, onDelete, onClear }: BatchActionBarProps) {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 z-40 flex -translate-x-1/2 items-center gap-4 rounded-xl border border-border bg-card px-5 py-3 shadow-2xl backdrop-blur-[24px]">
      ...
      <button
        type="button"
        onClick={onClear}
        className="flex items-center gap-1.5 rounded-md border border-border px-3 py-1.5 text-xs text-[var(--color-text-secondary)] transition-colors duration-150 hover:bg-white/5 focus:outline-none focus:ring-2 focus:ring-[var(--color-electric-purple)]/50"
      >
