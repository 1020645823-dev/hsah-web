    ...
    <div className="flex min-h-[40vh] items-center justify-center px-6 py-10">
      <div className="w-full max-w-lg rounded-2xl border border-border bg-card p-8 shadow-[var(--shadow-card)]">
        <div className="space-y-5">
          ...
          <div className="rounded-lg border border-border bg-muted/40 p-4 text-sm leading-6 text-foreground/75">
            {message}
          </div>

          <div className="grid gap-3 rounded-lg border border-border bg-muted/20 p-4 text-sm text-foreground/75 sm:grid-cols-2">
            <PanelFact icon={ArrowLeftRight} title={t("workspaceContext")} description={t("workspaceContextDesc")} />
            <PanelFact icon={Sparkles} title={t("libraryReturnPath")} description={t("libraryReturnPathDesc")} />
          </div>

          <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-foreground/60">
            <span>{t("publicLibraryAvailable")}</span>
